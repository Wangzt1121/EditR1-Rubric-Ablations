...
def _candidate_float_values(env_name, batch_size, device, dtype):
    ...
    if len(values) < batch_size:
        ...
        values = (values * repeats)[:batch_size]
    else:
...
def pipeline_with_logprob(
    ...
    elif height % multiple_of != 0 or width % multiple_of != 0:
        raise ValueError(
            f"Exact output size requires height and width to be divisible by {multiple_of}, "
            f"got height={height}, width={width}. Resize or filter this sample instead of "
            "letting the pipeline silently change the output size."
        )
    ...
    if self.transformer.config.guidance_embeds:
        guidance = _candidate_float_values(
            "EDIT_R1_CANDIDATE_GUIDANCE_SCALES",
            latents.shape[0],
            device,
            torch.float32,
        )
    ...
    candidate_noise_level = _candidate_float_values(
        "EDIT_R1_CANDIDATE_NOISE_LEVELS",
        latents.shape[0],
        latents.device,
        torch.float32,
    )
    sampling_noise_level = (
        candidate_noise_level if candidate_noise_level is not None else noise_level
    )
    ...
    latents, all_latents, all_log_probs = run_sampling(
        v_pred_fn, latents, sigmas, solver, deterministic, sampling_noise_level
    )

    latents = self._unpack_latents(latents, height, width, self.vae_scale_factor)
    latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
    latents = latents.to(dtype=self.vae.dtype)
    decode_batch_size = int(os.getenv("EDIT_R1_VAE_DECODE_BATCH_SIZE", "0") or 0)
    if decode_batch_size > 0 and latents.shape[0] > decode_batch_size:
        image = torch.cat(
            [
                self.vae.decode(latent_chunk, return_dict=False)[0]
