...
def run_sampling(
    ...
    for i in tqdm(
        ...
        if solver == "flow":
            flow_eta = eta if not determistic else 0
...
def flow_grpo_step(
    ...
    eta_t = eta
    if not torch.is_tensor(eta_t):
        eta_t = torch.as_tensor(eta_t, device=device, dtype=model_output.dtype)
    else:
        eta_t = eta_t.to(device=device, dtype=model_output.dtype)
    if eta_t.ndim == 1:
        eta_t = eta_t.view(-1, *([1] * (model_output.ndim - 1)))
    std_dev_t = (
