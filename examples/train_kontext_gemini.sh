...
export SINGLE_REWARD_MODEL=${SINGLE_REWARD_MODEL:-${GEMINI_REWARD_MODEL:-gemini-3.1-flash-lite}}
...
export EVAL_SAMPLE_COUNT=${EVAL_SAMPLE_COUNT:-50}
...
echo "[train_kontext] config=${CONFIG_NAME} mode=${SINGLE_REWARD_MODE} model=${SINGLE_REWARD_MODEL} rubric=${SINGLE_REWARD_RUBRIC_YAML} steps=${NUM_EPOCHS} lr=${LORA_LEARNING_RATE} eval_every=${EVAL_FREQ} eval_samples=${EVAL_SAMPLE_COUNT} gpus=${CUDA_VISIBLE_DEVICES}"
